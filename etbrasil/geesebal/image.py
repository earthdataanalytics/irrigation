...
class Image():
    ...
    def __init__(self,
        ...
        if self.landsat_version == 'LANDSAT_5':
            self.image = image.map(prepSrLandsat5and7)
            self.image=self.image.select(Constants.LANDSAT_5_7_BANDS["OFFICIAL"],
        ...
        #ALBEDO TASUMI ET AL. (2008)
            ...
            self.image=self.image.select(Constants.LANDSAT_5_7_BANDS["OFFICIAL"],
            Constants.LANDSAT_5_7_BANDS["CUSTOM"])
            self.image_toa=ee.Image(Constants.LANDSAT_COLLECTION_7 + "/"+ self._index.getInfo())
            ...
            self.image = image.map(prepSrLandsat8)
            self.image = self.image.select(Constants.LANDSAT_8_BANDS["OFFICIAL"],
        ...
        #SRTM DATA ELEVATION
        self.srtm = ee.Image(Constants.SRTM_ELEVATION_COLLECTION).clip(self.geometryReducer);
...
#        self.image=self.image.addBands([self.image.select('ET_24h').rename(self.NAME_FINAL)])
        ...
        #self.ux= col_meteorology.select('ux_G') #WIND SPEED [M S-1]
        #self.UR = col_meteorology.select('RH_G') #RELATIVE HUMIDITY [%]
        #self.elevation = self.srtm.select('elevation') #SRTM DATA ELEVATION
        #self.slope = ee.Terrain.slope('elevation')
