...
def fexp_spec_ind(image):
    ...
    # LEAF AREA INDEX (LAI)
    lai = image.expression("-(log(( 0.69-SAVI)/0.59 ) / 0.91)", {"SAVI": savi1}).rename(
        "LAI"
    )
    ...
    # TODO: This is no longer necessary. In collection 2 we have LST
...
def fexp_lst_export(img_main, img_main_RAD, landsat_version, refpoly):
    ...
    tairColl_wv = ee.ImageCollection(
        Constants.SURFACE_WATER_VAPOUR_COLLECTION
    ...
    # gamma = (
    #     radtemp.multiply(c2)
    #     .divide(brightemp.pow(2))
    #     .multiply(radtemp.multiply(lambda1.pow(4)).divide(c1).add(lambda1.pow(-1)))
    #     .pow(-1)
    # )
    ...
    # LStemp = (
    #     gamma.multiply(((psi1.multiply(radtemp)).add(psi2).divide(e)).add(psi3))
    #     .add(delta)
    #     .rename("T_LST")
    # )

    LStemp = img_main.select("T_LST")
...
def LST_DEM_correction(image, z_alt, T_air, UR, SUN_ELEVATION, hour, minuts):
    ...
    pres = image.expression(
        "101.3 * ((293 - (0.0065 * Z))/ 293) ** 5.26 ", {"Z": z_alt}
    ).rename("P_ATM")

    # SATURATION VAPOR PRESSURE (es) [KPA]
    es = image.expression(
        " 0.6108 *(exp( (17.27 * T_air) / (T_air + 237.3)))", {"T_air": T_air}
    ).rename("ES")
    ...
    W = image.expression("(0.14 * EA * PATM) + 2.1", {"PATM": pres, "EA": ea}).rename(
        "W_ATM"
    )
    ...
    degree2radian = ee.Number(0.01745)
    ...
    tao_sw = image.expression(
        ...
        {"P": pres, "W": W, "Kt": ee.Number(1), "cos_theta": cos_theta},
    ).rename("Tao_sw")

    # AIR DENSITY [KG M-3]
    air_dens = image.expression(
        "(1000* Pair)/(1.01*LST*287)", {"Pair": pres, "LST": image.select("T_LST")}
    )
    ...
    Temp_corr = image.select("T_LST").add(
        z_alt.select("elevation").multiply(Temp_lapse_rate)
    )
    ...
    delta = ee.Image(
        ...
        .sin()
        .asin()
        .multiply(B.multiply(degree2radian).sin())
    )
    ...
    b = (delta.cos().multiply(phi.cos()).multiply(s.cos())).add(
        delta.cos().multiply(phi.sin().multiply(s.sin()).multiply(gamma.cos()))
    )
...
def fexp_radlong_up(image):
    ...
    Rl_up = image.expression(
        "emi * stefBol * (LST ** 4)",
        {"emi": emi, "stefBol": stefBol, "LST": image.select("T_LST")},
    ).rename("Rl_up")
...
def fexp_radshort_down(image, z_alt, T_air, UR, SUN_ELEVATION):
    ...
    Rs_down = image.expression(
        ...
        {"gsc": gsc, "tao_sw": tao_sw, "dr": dr, "cos_theta": cos_theta},
    ).rename("Rs_down")
...
def fexp_radlong_down(image, n_Ts_cold):
    ...
    Rl_down = image.expression(
        ...
        {"log_taosw": log_taosw, "stefBol": ee.Number(5.67e-8), "n_Ts_cold": n_Ts_cold},
    ).rename("Rl_down")
    ...
    for n in range(15):
        ...
        i_dT_int = (
            ...
            .expression(
                ...
                {"n_coef_a": n_coef_a, "n_coef_b": n_coef_b, "i_lst_med": i_lst_med},
            )
            .rename("dT")
        )
        ...
        i_psim_200 = img.expression(
            "-5*(hight/i_L_int)", {"hight": ee.Number(200), "i_L_int": i_L_int}
        ).rename("psim_200")
        i_psih_2 = img.expression(
            "-5*(hight/i_L_int)", {"hight": ee.Number(2), "i_L_int": i_L_int}
        ).rename("psih_2")
        ...
        i_x200 = i_L_int.expression(
            ...
            {"hight": ee.Number(200), "i_L_int": i_L_int},
        ).rename("i_x200")
        i_x2 = i_L_int.expression(
            "(1-(16*(hight/i_L_int)))**0.25",
            {"hight": ee.Number(2), "i_L_int": i_L_int},
        ).rename("i_x2")
        ...
        if n > 1:
            ...
            n_dif = (
                n_dT_hot_abs.subtract(n_dT_hot_old_abs)
                .add(n_rah_hot_abs)
                .subtract(n_rah_hot_old_abs)
            ).abs()
            n_dT_hot_old = n_dT_hot
            n_rah_hot_old = n_rah_hot
