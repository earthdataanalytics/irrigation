exports.spec_ind = function (image, calculateTST) {
  ...
  var evi = image.expression('2.5 * ((N - R) / (N + (6 * R) - (7.5 * B) + 1))', {
    'N': image.select('NIR').multiply(1),
    'R': image.select('R').multiply(1),
    'B': image.select('B').multiply(1),
  }).rename('EVI');
  ...
  // TODO: CHECK IF THIS IS CORRECT WITH THE NEW VERSION OF LANDSAT8 COLLECTION
  // check if BAND T_LST exist:
  if (calculateTST == true) {
    // console.log("calculating TST")
    ...
    // SCCALE LST MULTIPLYING BY 0.00341802

  }
  var lst = image.select('T_LST')
  // .multiply(0.00341802);

  // GET MAX LST USING REDUCE REGION
  // var max_lst = lst.reduceRegion({
...
exports.LST_correction = function (image, z_alt, T_air, UR, SUN_ELEVATION, hour, min) {
  ...
  var lst_nw = TS_DEM.updateMask(ndwi.lte(0)).rename("LST_NW")
  var lst_neg = lst_nw.multiply(-1).rename("LST_neg")
  image = image.addBands([TS_DEM, lst_nw, lst_neg, cos_zn.rename('Solar_angle_cos')]);
