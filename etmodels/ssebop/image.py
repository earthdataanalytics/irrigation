...
class Image:
        ...
        **kwargs,
        ...
        if type(self._elr_flag) is str:
            if self._elr_flag.upper() in ["TRUE"]:
                ...
                raise ValueError(
                    'elr_flag "{}" could not be interpreted as '
    ...
    def et_fraction(self):
        ...
        if self.et_fraction_type.lower() == "grass":
            ...
            nldas_coll = ee.ImageCollection("NASA/NLDAS/FORA0125_H002").select(
                [
                    "temperature",
                    "specific_humidity",
                    "shortwave_radiation",
            ...
            nldas_prev_img = ee.Image(
                nldas_coll.filterDate(
                    time_start.subtract(2 * 60 * 60 * 1000), time_start
                )
            ...
            nldas_next_img = ee.Image(
                nldas_coll.filterDate(
                    time_start, time_start.add(2 * 60 * 60 * 1000)
                ).first()
            )
            ...
            time_ratio = time_start.subtract(nldas_prev_time).divide(
                nldas_next_time.subtract(nldas_prev_time)
            )
            nldas_img = (
                nldas_next_img.subtract(nldas_prev_img)
                .multiply(time_ratio)
                .add(nldas_prev_img)
            ...
            et_fraction = et_fraction.multiply(
                Hourly.nldas(nldas_img).etr
            ).divide(Hourly.nldas(nldas_img).eto)
    ...
    def et_reference(self):
        ...
        if utils.is_number(self.et_reference_source):
            ...
            if (
                self.et_reference_date_type is None
                ...
                raise ValueError(
                    "unsupported et_reference_date_type: {}".format(
        ...
        #         .filterDate(self.start_date, self.end_date)
            ...
            raise ValueError(
                "unsupported et_reference_source: {}".format(self.et_reference_source)
            )
    ...
    def dt(self):
        ...
        # Use precomputed dT median assets
        elif self._dt_source.lower().startswith(
            "projects/"
        ) or self._dt_source.lower().startswith("users/"):
            ...
            dt_coll = ee.ImageCollection(
                PROJECT_FOLDER + "/dt/daymet_median_v0"
            ...
            dt_coll = ee.ImageCollection(
                PROJECT_FOLDER + "/dt/daymet_median_v1"
            ).filter(ee.Filter.calendarRange(self._doy, self._doy, "day_of_year"))
            dt_img = ee.Image(dt_coll.first()).clamp(self._dt_min, self._dt_max)
        elif self._dt_source.upper() == "DAYMET_MEDIAN_V2":
            dt_coll = ee.ImageCollection(
                PROJECT_FOLDER + "/dt/daymet_median_v2"
            ).filter(ee.Filter.calendarRange(self._doy, self._doy, "day_of_year"))
            dt_img = ee.Image(dt_coll.first()).clamp(self._dt_min, self._dt_max)
            ...
            dt_img = model.dt(
                ...
                .multiply(input_img.select(["dayl"]))
                .divide(1000000),
    ...
    def tcorr(self):
        ...
        if utils.is_number(self._tcorr_source):
            return (
                ee.Image.constant(float(self._tcorr_source))
                .rename(["tcorr"])
                .set({"tcorr_index": tcorr_indices["user"]})
            )
        # Then check if Tmax source is a number but Tcorr is a spatial calculation
        elif utils.is_number(self._tmax_source) and (
            self._tcorr_source.upper() in ["DYNAMIC", "SCENE_GRIDDED"]
            or "SCENE" in self._tcorr_source.upper()
        ):
            raise ValueError(
        ...
        if "SCENE_GRIDDED" == self._tcorr_source.upper():
            ...
            scene_dict = {
                "DAYMET_MEDIAN_V2": f"{PROJECT_FOLDER}/tcorr_gridded/daymet_median_v2_scene",
                # f'daymet_v3_median_1980_2018':
                #     f'{PROJECT_FOLDER}/tcorr_gridded/daymet_median_v2_scene',
                f"daymet_v3_median_1980_2018": f"{PROJECT_FOLDER}/tcorr_gridded/c01/daymet_v3_median_1980_2018",
                f"daymet_v4_median_1980_2019": f"{PROJECT_FOLDER}/tcorr_gridded/c02/daymet_v4_median_1980_2019",
                f"daymet_v4_mean_1981_2010": f"{PROJECT_FOLDER}/tcorr_gridded/c02/daymet_v4_mean_1981_2010",
                f"daymet_v4_mean_1981_2010_elr": f"{PROJECT_FOLDER}/tcorr_gridded/c02/daymet_v4_mean_1981_2010_elr",
            }

            if self._tmax_source.upper() in scene_dict.keys():
                tmax_key = self._tmax_source.upper()
            elif (
                self._tmax_source.startswith("projects/")
                and self._tmax_source.split("/")[-1] in scene_dict.keys()
            ):
            ...
            "DYNAMIC" == self._tcorr_source.upper()
            or self._tcorr_source.upper().startswith("SCENE")
        ):
            # Use the precomputed scene monthly/annual climatologies if Tcorr
            #   can't be computed dynamically.
            scene_dict = {
                "DAYMET_MEDIAN_V2": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_scene",
                f"daymet_v3_median_1980_2018": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_scene",
            }
            month_dict = {
                "DAYMET_MEDIAN_V2": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_monthly",
                f"daymet_v3_median_1980_2018": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_monthly",
            }
            annual_dict = {
                "DAYMET_MEDIAN_V2": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_annual",
                f"daymet_v3_median_1980_2018": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_annual",
            }
            default_dict = {
                "DAYMET_MEDIAN_V2": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_default",
                f"daymet_v3_median_1980_2018": f"{PROJECT_FOLDER}/tcorr_scene/daymet_median_v2_default",
            }
            ...
            default_coll = ee.ImageCollection(default_dict[tmax_key]).filterMetadata(
                "wrs2_tile", "equals", self._wrs2_tile
            )
            annual_coll = (
                ee.ImageCollection(annual_dict[tmax_key])
                .filterMetadata("wrs2_tile", "equals", self._wrs2_tile)
                .select(["tcorr"])
            )
            month_coll = (
                ee.ImageCollection(month_dict[tmax_key])
                .filterMetadata("wrs2_tile", "equals", self._wrs2_tile)
        ...
        if re.match("projects/.+/tcorr_gridded/.+", self._tcorr_source):
            ...
            scene_coll = (
                ee.ImageCollection(self._tcorr_source)
                .filterDate(self._start_date, self._end_date)
                .filterMetadata("wrs2_tile", "equals", self._wrs2_tile)
                .select(["tcorr"])
            )
            ...
            if self._tcorr_resample.lower() in ["bilinear"]:
                tcorr_img = tcorr_img.resample(self._tcorr_resample.lower()).reproject(
                ...
                raise ValueError(
                    "Unsupported tcorr_resample: {}\n".format(self._tcorr_resample)
                )
            ...
            t_stats = ee.Dictionary(self.tcorr_stats).combine(
                {"tcorr_value": 0, "tcorr_count": 0}, overwrite=False
            )
            tcorr_value = ee.Number(t_stats.get("tcorr_value"))
            tcorr_count = ee.Number(t_stats.get("tcorr_count"))
            tcorr_index = tcorr_count.lt(min_pixels_per_image).multiply(
                tcorr_indices["nodata"]
            )
            ...
            scene_coll = (
                ee.ImageCollection(scene_dict[tmax_key])
                .filterDate(self._start_date, self._end_date)
                .filterMetadata("wrs2_tile", "equals", self._wrs2_tile)
                .select(["tcorr"])
            )
            ...
            raise ValueError(
                "Unsupported tcorr_source: {}\n".format(self._tcorr_source)
            )
    ...
    def tmax(self):
        ...
        if utils.is_number(self._tmax_source):
            ...
            tmax_image = (
                ee.Image.constant(float(self._tmax_source))
                .rename(["tmax"])
                .set({"tmax_source": "custom_{}".format(self._tmax_source)})
            )
        elif re.match(
            r"^projects/.+/tmax/.+_(mean|median)_\d{4}_\d{4}(_\w+)?", self._tmax_source
        ):
            ...
            tmax_image = ee.Image(tmax_coll.filter(doy_filter).first()).set(
                {"tmax_source": self._tmax_source}
            )
            ...
            tmax_coll = (
                ee.ImageCollection(f"NASA/ORNL/{self._tmax_source.upper()}")
            ...
            tmax_coll = (
                ee.ImageCollection(tmax_coll_id)
                .filterDate(self._start_date, self._end_date)
                .select(["Tx"], ["tmax"])
                .map(utils.c_to_k)
            )
    ...
    def from_landsat_c1_toa(cls, toa_image, cloudmask_args={}, **kwargs):
        ...
        prep_image = toa_image.select(input_bands.get(spacecraft_id), output_bands).set(
            {
        ...
        cloud_mask = common.landsat_c1_toa_cloud_mask(
    ...
    def tcorr_image(self):
        ...
        ndvi_buffer_mask = ndvi.gte(ndvi_threshold).reduceNeighborhood(
            reducer=ee.Reducer.min(), kernel=ee.Kernel.square(radius=60, units="meters")
        )
    ...
    def tcorr_gridded(self):
        ...
        tcorr_coarse_cold_img = (
            self.tcorr_image.reproject(crs=self.crs, crsTransform=self.transform)
            .reduceResolution(
                reducer=ee.Reducer.percentile(percentiles=[2.5]).combine(
        ...
        tcorr_coarse_hot_img = (
            self.tcorr_image_hot.reproject(crs=self.crs, crsTransform=self.transform)
            .reduceResolution(
                reducer=ee.Reducer.percentile(percentiles=[70]).combine(
                    reducer2=ee.Reducer.count(), sharedInputs=True
                ),
                bestEffort=False,
                maxPixels=30000,
        ...
        count_coarse_cold = tcorr_coarse_cold.reduceRegion(
        ...
        #                 .gte(self.min_pixels_per_grid_cell))
        tcorr_coarse_hot = tcorr_coarse_hot_img.select(["tcorr"]).updateMask(
            tcorr_coarse_hot_img.select(["count"])
        )

        # Count the number of coarse resolution Tcorr cells
        count_coarse_hot = tcorr_coarse_hot.reduceRegion(
            reducer=ee.Reducer.count(),
            crs=self.crs,
            crsTransform=coarse_transform,
            bestEffort=False,
        ...
        tcorr_rn01_cold = (
            tcorr_coarse_cold.reduceNeighborhood(
                reducer=ee.Reducer.mean(),
        ...
        tcorr_rn02_hot = (
            tcorr_coarse_hot.reduceNeighborhood(
                reducer=ee.Reducer.mean(),
                kernel=ee.Kernel.square(radius=2, units="pixels"),
                # kernel=ee.Kernel.square(radius=2, units='pixels'),
                # optimization='boxcar',
        ...
        tcorr_rn02_blended = (
            hotCold_mosaic.reduceNeighborhood(
                reducer=ee.Reducer.mean(),
                kernel=ee.Kernel.square(radius=2, units="pixels"),
                # kernel=ee.Kernel.square(radius=4, units='pixels'),
                # optimization='boxcar',
        ...
        fm_mosaic = ee.Image(
            [hotCold_mosaic, tcorr_rn02_cold, tcorr_rn04_blended, tcorr_rn16_blended]
        ).reduce(ee.Reducer.firstNonNull())
        ...
        total_score_img = ee.Image([score_null, score_02, score_04, score_16]).reduce(
            ee.Reducer.sum()
        )
        ...
        fm_mosaic_4 = (
            ee.Image(
                [
                    hotCold_mosaic.multiply(0.4).updateMask(1),
        ...
        fm_mosaic_3 = (
            ee.Image(
                [
                    tcorr_rn02_blended.multiply(0.5).updateMask(1),
        ...
        fm_mosaic_2 = (
            ee.Image(
                [
                    tcorr_rn04_blended.multiply(0.67).updateMask(1),
        ...
        tcorr = (
            ee.Image([fm_mosaic_4, fm_mosaic_3, fm_mosaic_2, fm_mosaic_1])
            .reduce(ee.Reducer.firstNonNull())
            .updateMask(1)
        )

        tcorr = (
            tcorr.reduceNeighborhood(
                reducer=ee.Reducer.mean(),
        ...
        quality_score_img = ee.Image(
            [total_score_img, hotscore, coldscore, cold_rn05_score]
        ).reduce(ee.Reducer.sum())
        ...
        return (
            ee.Image([tcorr, quality_score_img])
            .rename(["tcorr", "quality"])
            .set(self._properties)
            .set(
                {
    ...
    def tcorr_gridded_cold(self):
        ...
        # Count the number of coarse resolution Tcorr cells
        count_coarse = tcorr_coarse_img.reduceRegion(
            reducer=ee.Reducer.count(),
            crs=self.crs,
            crsTransform=coarse_transform,
            bestEffort=False,
        ...
        tcorr_rn02 = (
            tcorr_coarse.reduceNeighborhood(
                reducer=ee.Reducer.mean(),
                kernel=ee.Kernel.square(radius=2, units="pixels"),
                skipMasked=False,
            )
        ...
        #   proportionally to how close it is to the original c ---
        fm_mosaic = ee.Image(
            [tcorr_coarse, tcorr_rn02, tcorr_rn04, tcorr_rn16, tcorr_rn64]
        ).reduce(ee.Reducer.firstNonNull())
        zero_img = fm_mosaic.multiply(0).updateMask(1)
        ...
        # This will help us to know where to apply different weights as directed by G. Senay.
        total_score_img = ee.Image(
            [score_coarse, score_02, score_04, score_16, score_64]
        ).reduce(ee.Reducer.sum())
        ...
        fm_mosaic_5 = (
            ee.Image(
                [
                    tcorr_coarse.multiply(0.4).updateMask(1),
        ...
        fm_mosaic_4 = (
            ee.Image(
                [
                    tcorr_rn02.multiply(0.4).updateMask(1),
        ...
        fm_mosaic_3 = (
            ee.Image(
                [
                    tcorr_rn04.multiply(0.5).updateMask(1),
        ...
        fm_mosaic_2 = (
            ee.Image(
                [
                    tcorr_rn16.multiply(0.67).updateMask(1),
        ...
        # Combine the weighted means into a single image using first non-null
        tcorr = ee.Image(
            [fm_mosaic_5, fm_mosaic_4, fm_mosaic_3, fm_mosaic_2, fm_mosaic_1]
        ).reduce(ee.Reducer.firstNonNull())
        ...
        return (
            ...
            .rename(["tcorr", "quality"])
            .set(self._properties)
            .set({"tcorr_index": tcorr_index, "tcorr_coarse_count": tcorr_count})
        )
