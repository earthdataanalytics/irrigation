...
try:
    import core_interpolate as core_interpolate
    import utils
except ImportError:
    import sys
    import os
...
def from_scene_et_fraction(scene_coll, start_date, end_date, variables,
    ...
    t_interval : {'daily', 'monthly', 'annual', 'custom'}
        Time interval over which to core_interpolate and aggregate values
    ...
    use_joins : bool, optional
        ...
        This parameter is passed through to core_interpolate.daily().
    _interp_vars : list, optional
        The variables that can be interpolated to daily timesteps.
        The default is to core_interpolate the 'et_fraction' and 'ndvi' bands.
    ...
    # With the current core_interpolate.daily() function,
    ...
    if 'count' in variables:
        aggregate_coll = core_interpolate.aggregate_to_daily(
    ...
    daily_coll = core_interpolate.daily(
    ...
    if t_interval.lower() == 'daily':
        def agg_daily(daily_img):
            ...
            # It should be since it is coming from the core_interpolate source
