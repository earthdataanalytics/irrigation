...
def test_Image_init_scene_id_property():
    ...
    m = Image(input_img.set('system:index', '1_2_' + SCENE_ID))
    assert utils.getinfo(m._scene_id) == SCENE_ID
...
def test_Image_from_landsat_c1_toa_default_image():
    ...
    output = Image.from_landsat_c1_toa(ee.Image(f'{COLL_ID}/{SCENE_ID}'))
...
def test_Image_from_landsat_c1_toa_image_id(image_id):
    ...
    output = utils.getinfo(Image.from_landsat_c1_toa(image_id).ndvi)
...
def test_Image_from_landsat_c1_toa_image():
    ...
    output = utils.getinfo(Image.from_landsat_c1_toa(ee.Image(image_id)).ndvi)
    assert output['properties']['system:index'] == image_id.split('/')[-1]
...
def test_Image_from_landsat_c1_toa_exception():
    with pytest.raises(Exception):
        ...
        Image.from_landsat_c1_toa(ee.Image('FOO')).ndvi.getInfo()


def test_Image_from_landsat_c1_toa_reflectance_type():
    """Test if reflectance_type property is being set"""
    image_id = 'LANDSAT/LC08/C01/T1_TOA/LC08_044033_20170716'
    assert Image.from_landsat_c1_toa(image_id).reflectance_type.upper() == 'TOA'


def test_Image_from_landsat_c1_sr_default_image():
    """Test that the classmethod is returning a class object"""
    output = Image.from_landsat_c1_sr(ee.Image(f'{COLL_ID}/{SCENE_ID}'))
    assert type(output) == type(default_image_obj())
...
def test_Image_from_landsat_c1_sr_image():
    ...
    output = utils.getinfo(Image.from_landsat_c1_sr(ee.Image(image_id)).ndvi)
    assert output['properties']['system:index'] == image_id.split('/')[-1]
...
def test_Image_from_landsat_c1_sr_exception():
    ...
    with pytest.raises(Exception):
        # Intentionally using .getInfo()
        Image.from_landsat_c1_sr(ee.Image('FOO')).ndvi.getInfo()


def test_Image_from_landsat_c1_sr_reflectance_type():
    """Test if reflectance_type property is being set"""
    image_id = 'LANDSAT/LC08/C01/T1_SR/LC08_044033_20170716'
    assert Image.from_landsat_c1_sr(image_id).reflectance_type.upper() == 'SR'
...
def test_Image_from_landsat_c1_sr_scaling():
    ...
    output = utils.constant_image_value(Image.from_landsat_c1_sr(input_img).lst)
...
def test_Image_from_landsat_c2_sr_default_image():
    """Test that the classmethod is returning a class object"""
    output = Image.from_landsat_c2_sr(ee.Image(f'{COLL_ID}/{SCENE_ID}'))
    assert type(output) == type(default_image_obj())
...
def test_Image_from_landsat_c2_sr_image_id(image_id):
    """Test instantiating the class from a Landsat SR image ID"""
    output = utils.getinfo(Image.from_landsat_c2_sr(image_id).ndvi)
    assert output['properties']['system:index'] == image_id.split('/')[-1]
...
def test_Image_from_landsat_c2_sr_image():
    ...
    output = utils.getinfo(Image.from_landsat_c2_sr(
...
def test_Image_from_landsat_c2_sr_exception():
    ...
    with pytest.raises(Exception):
        # Intentionally using .getInfo()
        Image.from_landsat_c2_sr(ee.Image('FOO')).ndvi.getInfo()
...
def test_Image_from_landsat_c2_sr_scaling():
    ...
    output = utils.constant_image_value(Image.from_landsat_c2_sr(input_img).ndvi)
    assert abs(output['ndvi'] - 0.333) <= 0.01

    output = utils.constant_image_value(Image.from_landsat_c2_sr(input_img).lst)
...
def test_Image_from_landsat_c2_sr_cloud_mask_args():
    ...
    image_id = 'LANDSAT/LC08/C02/T1_L2/LC08_044033_20170716'
    output = Image.from_landsat_c2_sr(
...
def test_Image_from_landsat_c2_sr_c2_lst_correct_arg():
    ...
    output = Image.from_landsat_c2_sr(image_id, c2_lst_correct=True)
    assert type(output) == type(default_image_obj())
...
def test_Image_from_landsat_c2_sr_c2_lst_correct_fill():
    ...
    uncorrected = utils.point_image_value(
        Image.from_landsat_c2_sr(image_id, c2_lst_correct=False).lst, point_xy)
    assert uncorrected['lst'] is None
    corrected = utils.point_image_value(
        Image.from_landsat_c2_sr(image_id, c2_lst_correct=True).lst, point_xy)
...
def test_Image_from_method_kwargs():
    ...
    assert Image.from_landsat_c1_toa(
    ...
    assert Image.from_landsat_c1_sr(
        ...
        elev_source='DEADBEEF')._elev_source == 'DEADBEEF'
    assert Image.from_landsat_c2_sr(
...
def test_Image_tcorr_stats_landsat(image_id, tmax_source, expected,
                                   tol=0.000001):
    output = utils.getinfo(Image.from_image_id(
...
#     m = Image(input_image, tcorr_source='IMAGE',
...
def test_Image_tcorr_gridded_method(tcorr_source, tmax_source, image_id,
    ...
    tcorr_img = Image.from_landsat_c2_sr(
...
#     point_xy = utils.getinfo(
#         ee.Geometry.Point(xy, image_crs).transform('EPSG:4326', 1).coordinates())
#     tcorr_img = Image.from_landsat_c1_toa(
#         ee.Image(image_id).clip(clip_geom), tcorr_source=tcorr_source,
#         tmax_source=tmax_source, tmax_resample='nearest').tcorr
#     tcorr = utils.point_image_value(tcorr_img, point_xy)
#     index = utils.getinfo(tcorr_img.get('tcorr_index'))
...
#         'system:index': 'LC08_042035_20150713',
#         'system:time_start': ee.Date('1980-07-13').millis()})
#     m = Image(input_image, tcorr_source='SCENE',
...
def test_Image_tcorr_source_scene_daily():
    ...
    input_image = ee.Image.constant(1).set({
        'system:index': 'LC08_042035_20150713',
        'system:time_start': ee.Date('1980-07-04').millis()})
    m = Image(input_image, tcorr_source='SCENE_DAILY',
                     tmax_source='DAYMET_MEDIAN_V2')
    tcorr_img = m.tcorr
    tcorr = utils.point_image_value(tcorr_img, SCENE_POINT)
    index = utils.getinfo(tcorr_img.get('tcorr_index'))
...
def test_Image_from_landsat_c1_toa_et_fraction():
    ...
    image_id = 'LANDSAT/LC08/C01/T1_TOA/LC08_044033_20170716'
    output = utils.getinfo(Image.from_landsat_c1_toa(image_id).et_fraction)
    assert output['properties']['system:index'] == image_id.split('/')[-1]
...
def test_Image_from_landsat_c1_sr_et_fraction():
    """Test if ETf can be built for a Landsat SR image"""
    image_id = 'LANDSAT/LC08/C01/T1_SR/LC08_044033_20170716'
    output = utils.getinfo(Image.from_landsat_c1_sr(image_id).et_fraction)
    assert output['properties']['system:index'] == image_id.split('/')[-1]
...
def test_Image_from_landsat_c2_sr_et_fraction():
    """Test if ETf can be built for a Landsat SR image"""
    image_id = 'LANDSAT/LC08/C02/T1_L2/LC08_044033_20170716'
    output = utils.getinfo(Image.from_landsat_c2_sr(image_id).et_fraction)
    assert output['properties']['system:index'] == image_id.split('/')[-1]
...
def test_Image_from_landsat_c1_toa_et():
    ...
    image_id = 'LANDSAT/LC08/C01/T1_TOA/LC08_044033_20170716'
    output = utils.getinfo(Image.from_landsat_c1_toa(
...
def test_Image_qa_water_mask(image_id, test_point, expected):
    ...
    output_img = Image.from_image_id(image_id)
...
def test_Image_et_reference_water_nodata(image_id, xy):
    ...
    output = utils.point_image_value(Image.from_image_id(
        image_id, et_reference_source='IDAHO_EPSCOR/GRIDMET',
